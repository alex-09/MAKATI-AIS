...
class UpdateObligationRepository{
    ...
    public function update($request){
        ...
        $update = DMBudgetaryObligationsTransac::where('furs_id', $request->tf_obli_id)->first();
        $update->update([   
            'type' => 'Update', 
            'status' => 'For Review', 
        ]);

        $update1 = TfObigation::where('tf_obli_id', $request->tf_obli_id)->first();
        $update1->update([
        ...
        for($x=0; $x<count($request->latest_balance); $x++){
            ...
            ->where('tf_count_id', 'counter_'.$x)->first();
            $update2->update([
                'type' => 'Update', 
