...
export default {
    ...
    methods: {
        fetchLists() {
            axios.get("trustfunds/forReviewTr")
                .then((response) => {
                    this.lists = response.data.list;
                    console.log(this.lists);
                    if (Array.isArray(response.data.list)) {
                        this.lists = response.data.list.filter((list) => {
                            return list.status === 'For Approval - DH';
                        });
                    }
                })
                .catch((error) => {
        ...
        pushToListView(listId, listTF, listStr) {
            console.log('List to be pushed:', listId, listTF, listStr);
            
            const router = useRouter();
            router.push({
                name: 'List of Trust Receipts on Process - Review', // Replace with the actual name of the target route
                query: {
                id: listId,
                tf: listTF,
                str: listStr
    ...
    mounted() {
        ...
        .then((response) => {
            ...
            if (Array.isArray(response.data.list)) {
                this.lists = response.data.list.filter((list) => {
                    return list.status === 'For Approval - CA';
                });
            }
        })
        .catch((error) => {
    ...
    computed: {
        filteredLists() {
            ...
            const filtered = this.lists.filter((list) => {
                return (
                    list.str_name.toLowerCase().includes(searchQuery) &&
                    list.status === 'For Approval - CA'
                );
            });
    ...
    setup() {
        ...
        const pushToListView = (listId, listTF, listStr) => {
        console.log('List to be pushed:', listId, listTF, listStr);
...
<template>
    <div class="flex justify-between items-center m-8">
        <span class="text-[22px] font-bold text-[#3A3541]">List of Trust Receipts on Process</span>
        <div class="mx-3 my-3">
    ...
    <div class="flex justify-center">
        <div class="overflow-x-auto overscroll-x-contain h-[500px] w-1850 1536:w-1458 xxxxl:w-1850 xxxxl:h-[500px]">
            <table class="divide-y w-[2500px] divide-gray-200 mr-8">
                <thead>
                    <tr>
                        <th
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-if="filteredLists.length === 1 && filteredLists[0].message">
                    ...
                    <tr v-else v-for="list in filteredLists" :key="list.id">
                        ...
                        <td
                            class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap text-center capitalize">
                            {{ list.amount_allocated }}
                        </td>
                        <td
                            class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap text-center capitalize">
                            <div v-if="list.processor === null">
                                N/A
                            </div>
                            {{ list.processor }}
                        </td>
                        ...
                        <td class="px-2 py-4 whitespace-nowrap text-center w-207">
                            <div class="flex flex-row gap-2 justify-center">
                                <button
                                    @click="pushToListView(list.type, list.tf_id, list.str_name)"
