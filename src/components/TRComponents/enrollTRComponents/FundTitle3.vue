  ...
  methods: {
    async saveTR() {
      ...
      if (check == true) {
        ...
        await axios
          .post("trustfunds/enrolltransfer", {
            fund_data: fundData,
            government_type: this.trustReceipt.typeOfGovernment,
            agency_name: this.trustReceipt.nameOfAgency,
            document_source: this.trustReceipt.selectedFile,
            general_description: this.trustReceipt.generalDescription,
            nadai_no: this.trustReceipt.nadaiNo,
            nadai_date: this.trustReceipt.nadaiDate,
          ...
          .then((res) => {
            localStorage.setItem("toga_id", res.data.toga_id);
    ...
    async addTR(index) {
      ...
      if (this.validation() == true) {
        ...
        await axios
          .post("trustfunds/transferAddSubFund", {
            fund_data: fundData,
            tfid: localStorage.getItem("toga_id"),
          })
          .then((res) => {
            localStorage.setItem("toga_id", res.data.toga_id);
            (this.forRev = true), this.$emit("data-to-parent", this.forRev);
      ...
      for (let fundkey in this.fundForms) {
        ...
        for (let mainkey in funditem.mainFundForms) {
          ...
          for (let key in mainitem.subFundForms) {
            ...
            if (
              this.trustReceipt.typeOfGovernment == "" ||
              this.trustReceipt.nameOfAgency == "" ||
              this.trustReceipt.selectedFile == null ||
              this.trustReceipt.generalDescription == "" ||
              this.trustReceipt.nadaiNo == "" ||
              this.trustReceipt.nadaiDate == "" ||
    ...
    forReview() {
      axios
        .post("trustfunds/transferForReview", {
          tfid: localStorage.getItem("toga_id"),
        })
        .then(
          localStorage.removeItem("toga_id")(
            (this.forRev = false)
