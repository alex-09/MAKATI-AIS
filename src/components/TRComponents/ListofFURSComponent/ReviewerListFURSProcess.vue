...
export default {
    ...
    methods: {
        fetchLists() {
            axios.get("trustfunds/forReviewFurs")
                .then((response) => {
        ...
        pushToListView(listId) {
            console.log('List to be pushed:', listId);
    ...
    computed: {
        filteredLists() {
            ...
            const filtered = this.lists.filter((list) => {
                return (
                    list.transaction_id.toLowerCase().includes(searchQuery) &&
...
<template>
    <div class="flex justify-between items-center m-8">
        <span class="text-[22px] font-bold text-[#3A3541]">List of FURS on Process</span>
        <div class="mx-3 my-3">
        ...
        <div class="overflow-x-auto overscroll-x-contain h-[500px] w-1850 1536:w-1458 xxxxl:w-1850 xxxxl:h-[500px]">
            <table class="divide-y w-[2500px] divide-gray-200 mr-8">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr v-else v-for="list in filteredLists" :key="list.transaction_id">
                        <td
                        ...
                        <td class="px-2 py-4 whitespace-nowrap text-center w-207">
                            <div class="flex flex-row gap-2 justify-center">
                                <button
                                    @click="pushToListView(list.transaction_id)"
