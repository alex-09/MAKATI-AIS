<template>
    ...
    <div class=" px-3 flex flex-1 flex-col sm:flex-row justify-between items-center text-other-black font-Inter text-xs font-semibold border shadow-md">
        <div class="flex flex-row items-center text-primary-color">
            ...
            <span class="mx-8 text-primary-color">Effectivity date: {{ currentDates }} {{ coa_title }}</span>
        </div>
        <div class="mx-3 my-3">
            ...
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8">
                ...
                v-if="this.userDetails === 'City Accountant' || userPosition === 'Assistant Department Head forFRS'">
                    <tr v-if="filteredAssets.length === 1 && filteredAssets[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredAssets[0].message }}</td>
                    </tr>
                    <tr v-else v-for="asset in filteredAssets" :key="asset.id">
                        ...
                        <td v-if="asset.status == 'enable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">       
                            <button @click="disableAcc(asset.id)"  class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-save-color">enable</button>
                        </td>
                        <td v-if="asset.status == 'disable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">
                            <button @click="enableAcc(asset.id)" class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-danger-color">disabled</button>
                        </td>
                    </tr>
                </tbody>
                <tbody 
                ...
                v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division' || this.userDetails === 'Processor' || this.userDivision === 'Financial Reporting Division'">
                    <tr v-if="filteredAssetsEnable.length === 1 && filteredAssetsEnable[0].message">
                        <td colspan="3" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredAssetsEnable[0].message }}</td>
                    </tr>
                    <tr v-else v-for="asset in filteredAssetsEnable" :key="asset.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ asset.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ asset.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(asset)" :euser="asset"><ViewIcon /></button></td>
                    </tr>
...
import viewDesc from '../modal/sampleModal.vue';
...
export default {
    ...
    methods: {
        ...
        async updateDesc(id) {
            try {
                const response = await axios.post(`coa/addAssetDescription/${id}`, {
        ...
        enableAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/updateAssetStatus/${id}`)
                        .then(response => {
    ...
    mounted() {
        ...
        this.description = this.euser.currDesc.description;
            ...
            axios.get('coa/showAssets').then(response => this.currentDates = response.data.date[0]['date_effectivity'])
            axios.get('coa/showAssets').then(response => this.coa_title = response.data.list[0]['coa_title'])
    },
    computed: {
        filteredAssets() {
            ...
            const filtered = this.assets.filter((asset) => {
                ...
                ) && asset.status !== 'For Approval - DH' && asset.status !== 'For Approval - CA';
            });
        ...
        filteredAssetsEnable() {
            const searchQuery = this.searchQuery.toLowerCase();
            const filtered = this.assets.filter((asset) => {
                return (
                asset.account_code.toLowerCase().includes(searchQuery) ||
                asset.account_title.toLowerCase().includes(searchQuery)
                ) && asset.status !== 'disable' && asset.status !== 'For Approval - DH' && asset.status !== 'For Approval - CA';
            });
