            ...
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8">
                ...
                v-if="this.userDetails === 'City Accountant' || userPosition === 'Assistant Department Head forFRS'">
                    ...
                    <tr v-else v-for="expense in filteredExpenses" :key="expense.id">
                        ...
                        <td v-if="expense.status == 'enable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">       
                            <button @click="disableAcc(expense.id)"  class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-save-color">enable</button>
                        </td>
                        <td v-if="expense.status == 'disable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">
                            <button @click="enableAcc(expense.id)" class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-danger-color">disabled</button>
                        </td>
                    </tr>
                </tbody>
                <tbody 
                ...
                v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division' || this.userDetails === 'Processor' || this.userDivision === 'Financial Reporting Division'">
                    <tr v-if="filteredExpensesEnable.length === 1 && filteredExpensesEnable[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredExpensesEnable[0].message }}</td>
                    </tr>
                    <tr v-else v-for="expense in filteredExpensesEnable" :key="expense.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ expense.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ expense.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(expense)" :euser="expense"><ViewIcon /></button></td>
                    </tr>
...
export default {
    ...
    methods: {
        ...
        async updateDesc(id) {
            try {
                const response = await axios.post(`coa/addExpensesDescription/${id}`, {
        ...
        enableAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/updateExpensesStatus/${id}`)
                    .then(response => {
                        console.log(response.data);
                        window.location.reload();
                    })
        ...
        axios.get('coa/showExpenses')
            .then(response => {
                ...
                if (Array.isArray(response.data.list)) {
                    this.expense = response.data.list.filter(expense => {
                        return expense.status === 'enable' || expense.status === 'disable';
                    });
                }
            })
            .catch(error => {
    ...
    mounted() {
        ...
        this.description = this.euser.currDesc.description;
            ...
            axios.get('coa/showExpenses').then(response => this.currentDate = response.data.date[0]['date_effectivity'])
            axios.get('coa/showExpenses').then(response => this.coa_title = response.data.list[0]['coa_title'])
    },
    computed: {
        filteredExpenses() {
            ...
            const filtered = this.expenses.filter((expense) => {
                ...
                ) && expense.status !== 'For Approval - DH' && expense.status !== 'For Approval - CA';
            });
        ...
        filteredExpensesEnable() {
            const searchQuery = this.searchQuery.toLowerCase();
            const filtered = this.expenses.filter((expense) => {
                return (
                    expense.account_code.toLowerCase().includes(searchQuery) ||
                    expense.account_title.toLowerCase().includes(searchQuery)
                ) && expense.status !== 'disable' && expense.status !== 'For Approval - DH' && expense.status !== 'For Approval - CA';
            });
