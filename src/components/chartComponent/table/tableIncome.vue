            ...
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8">
                ...
                v-if="this.userDetails === 'City Accountant' || userPosition === 'Assistant Department Head forFRS'">
                    ...
                    <tr v-else v-for="income in filteredIncomes" :key="income.id">
                        ...
                        <td v-if="income.status == 'enable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">       
                            <button @click="disableAccount(income.id)"  class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-save-color">enable</button>
                        </td>
                        <td v-if="income.status == 'disable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">
                            <button @click="enableAccount(income.id)" class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-danger-color">disabled</button>
                        </td>
                    </tr>
                </tbody>
                <tbody 
                ...
                v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division' || this.userDetails === 'Processor' || this.userDivision === 'Financial Reporting Division'">
                    <tr v-if="filteredIncomesEnable.length === 1 && filteredIncomesEnable[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredIncomesEnable[0].message }}</td>
                    </tr>
                    <tr v-else v-for="income in filteredIncomesEnable" :key="income.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ income.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ income.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(income)" :euser="income"><ViewIcon /></button></td>
                    </tr>
...
export default {
    ...
    methods: {
        ...
        async updateDesc(id) {
            try {
                const response = await axios.post(`coa/addIncomeDescription/${id}`, {
        ...
        enableAccount(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/updateIncomeStatus/${id}`)
                    .then(response => {
                        console.log(response.data);
                        window.location.reload();
                        console.log(id);
        ...
        fetchIncomes() {
        axios.get('coa/showIncome')
            .then(response => {
                this.currentDate = response.data.date[0]['date_effectivity'];
                this.coa_title = response.data.list[0]['coa_title'];
                if (Array.isArray(response.data.list)) {
                    this.income = response.data.list.filter(income => {
                        return income.status === 'enable' || income.status === 'disable';
                    });
                }
            })
            .catch(error => {
    ...
    mounted() {
        ...
        this.description = this.euser.currDesc.description;
            axios.get('coa/showIncome')
            .then(response => this.incomes = response.data.list)

            axios.get('coa/showIncome').then(response => this.currentDate = response.data.date[0]['date_effectivity'])
            axios.get('coa/showIncome').then(response => this.coa_title = response.data.list[0]['coa_title'])
    },
    computed: {
        filteredIncomes() {
            ...
            const filtered = this.incomes.filter((income) => {
                ...
                ) && income.status !== 'For Approval - DH' && income.status !== 'For Approval - CA';
            });
        ...
        filteredIncomesEnable() {
            const searchQuery = this.searchQuery.toLowerCase();
            const filtered = this.incomes.filter((income) => {
                return (
                    income.account_code.toLowerCase().includes(searchQuery) ||
                    income.account_title.toLowerCase().includes(searchQuery)
                ) && income.status !== 'disable' && income.status !== 'For Approval - DH' && income.status !== 'For Approval - CA';
            });
