        ...
        <div class="overflow-y-auto overscroll-y-auto overflow-x-hidden w-1850 1536:w-1458 1536:h-[470px] xxxxl:w-1850 max-h-[500px] xxxxl:h-[470px]">
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8">
                ...
                v-if="this.userDetails === 'City Accountant' || userPosition === 'Assistant Department Head forFRS'">
                    <tr v-if="filteredEquities.length === 1 && filteredEquities[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredEquities[0].message }}</td>
                    </tr>
                    <tr v-else v-for="equity in filteredEquities" :key="equity.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ equity.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ equity.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(equity)" :euser="equity"><ViewIcon /></button></td>
                        <td v-if="equity.status == 'enable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">       
                            <button @click="disableAcc(equity.id)"  class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-save-color">enable</button>
                        </td>
                        <td v-if="equity.status == 'disable'" class="px-6 py-4 text-sm text-gray-800 text-center uppercase">
                            <button @click="enableAcc(equity.id)" class="w-100 h-30 rounded uppercase text-base font-semibold text-white bg-danger-color">disabled</button>
                        </td>
                    </tr>
                </tbody>
                <tbody 
                ...
                v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division' || this.userDetails === 'Processor' || this.userDivision === 'Financial Reporting Division'">
                    <tr v-if="filteredEquitiesEnable.length === 1 && filteredEquitiesEnable[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredEquitiesEnable[0].message }}</td>
                    </tr>
                    <tr v-else v-for="equity in filteredEquitiesEnable" :key="equity.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ equity.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ equity.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(equity)" :euser="equity"><ViewIcon /></button></td>
                    </tr>
...
export default {
    ...
    methods: {
        ...
        initView(equity) {
            this.euser = { currDesc: equity };
            this.showViewDesc = true;
        },
        async updateDesc(id) {
            try {
                const response = await axios.post(`coa/addEquityDescription/${id}`, {
        ...
        enableAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/updateEquityStatus/${id}`)
                    .then(response => {
                        console.log(response.data);
                        window.location.reload();
                    })
        ...
        axios.get('coa/showEquity')
            .then(response => {
                ...
                if (Array.isArray(response.data.list)) {
                    this.equity = response.data.list.filter(equity => {
                        return equity.status === 'enable' || equity.status === 'disable';
                    });
                }
            })
            .catch(error => {
    ...
    mounted() {
        ...
        .then(response => {
            if (Array.isArray(response.data.list)) {
                this.equities = response.data.list.filter(equity => {
                    return equity.status === 'enable' || equity.status === 'disable';
                });
            }
        ...
        this.description = this.euser.currDesc.description;
            axios.get('coa/showEquity')
            .then(response => this.equities = response.data.list)

            axios.get('coa/showEquity').then(response => this.currentDate = response.data.date[0]['date_effectivity'])
            axios.get('coa/showEquity').then(response => this.coa_title = response.data.list[0]['coa_title'])
    },
    computed: {
        filteredEquities() {
            ...
            const filtered = this.equities.filter((equity) => {
                return (
                    equity.account_code.toLowerCase().includes(searchQuery) ||
                    equity.account_title.toLowerCase().includes(searchQuery)
                ) && equity.status !== 'For Approval - DH' && equity.status !== 'For Approval - CA';
            });
        ...
        filteredEquitiesEnable() {
            const searchQuery = this.searchQuery.toLowerCase();
            const filtered = this.equities.filter((equity) => {
                return (
                    equity.account_code.toLowerCase().includes(searchQuery) ||
                    equity.account_title.toLowerCase().includes(searchQuery)
                ) && equity.status !== 'disable' && equity.status !== 'For Approval - DH' && equity.status !== 'For Approval - CA';
            });
