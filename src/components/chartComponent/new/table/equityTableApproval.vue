            ...
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8">
                ...
                v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division'">
                    <tr v-if="filteredEquities.length === 1 && filteredEquities[0].message">
                        <td colspan="3" class="px-6 py-4 text-sm font-medium text-gray-800 text-center uppercase">{{ filteredEquities[0].message }}</td>
                    </tr>
                    <tr v-else v-for="equity in filteredEquities" :key="equity.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">
                            <input type="checkbox" :value="equity.id" v-model="selected" @click="toggleSelected"/>   
                        </td>
...
export default {
    ...
    methods: {
        ...
        approveAcc() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const selectedEquities = this.selected;

                    // Check if any Liabilities is selected
                    if (selectedEquities.length === 0) {
                        alert("Please select at least one equity to approve.");
                        return;
                    }

                    // Send approval request for each selected Liabilities
                    axios.post(`coa/equityforApprovalCa`, { id: this.selected })
                    .then((response) => {
                        console.log(`Equities ${response.data.id} has been approved.`);
                        window.location.reload();
                    })
                    .catch((error) => {
                        console.error(`Error approving equity ${error.message}`);
                    });
        ...
        disapproveAcc() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.post(`coa/disapproveEquity`, { id: this.selected })
                    .then((response) => {
                        console.log(`Equity ${response.data.id} has been disapproved.`);
                        window.location.reload();
                    })
                    .catch((error) => {
                        console.error(`Error approving Equity ${error.message}`);
        ...
        toggleSelectAll() {
            ...
            if (this.selectAll) {
                this.selected = this.filteredEquities.map((equity, index) => ({ [`id[${index}]`]: equity.id })); // create array of objects with each object containing the id of the corresponding liability
        ...
        toggleSelected(equity) {
            console.log(equity)
            if (this.isSelected(equity)) {
                const index = this.selected.findIndex((selectedEquity) => {
                    console.log(selectedEquity);
                    return selectedEquity.id === equity.id;
                });
                this.selected.splice(index, 1);
            } else {
                this.selected.push(equity.id);
            }
                this.selectAll = this.selected.length === this.equities.length;
        },
    },
    watch: {
        selectAll(newVal) {
            if (newVal) {
            this.selected = this.filteredEquities.map(equity => equity.id);
            } else {
            this.selected = [];
            }
        }
    ...
    mounted() {
        ...
        this.description = this.euser.currDesc.description;
            ...
            axios.get('coa/listTempEquity').then(response => this.selectedOption = response.data.list[0]['date_effectivity'])
            axios.get('coa/listTempEquity').then(response => this.coa_title = response.data.list[0]['coa_title'])
    ...
    computed: {
        filteredEquities() {
            ...
            const filtered = this.equities.filter((equity) => {
                ...
                ) && equity.approval_status !== 'For Approval - CA';
            });
