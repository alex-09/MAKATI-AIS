        ...
        <div class="overflow-y-auto overscroll-y-auto overflow-x-hidden w-1850 1536:w-1458 xxxxl:w-1850 max-h-[700px]">
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8">
                <thead>
                    ...
                    <tr v-else-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division'">
                        <th 
                            scope="col" 
                ...
                v-if="this.userDetails === 'Processor' && this.userDivision === 'Financial Reporting Division' || this.userDetails === 'Reviewer' && this.userDivision === 'Financial Reporting Division'" >
                    <tr v-if="filteredAssets.length === 1 && filteredAssets[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ filteredAssets[0].message }}</td>
                    </tr>
                    <tr v-else v-for="asset in filteredAssets" :key="asset.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ asset.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ asset.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(asset)" :euser="asset"><ViewIcon /></button></td>
                    </tr>
                </tbody>

                <!-- TBODY OF DIVISION HEAD -->
                <tbody 
                class="divide-y divide-gray-200"
                v-else-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division'">
                    <tr v-if="filteredAssets.length === 1 && filteredAssets[0].message">
                        <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ filteredAssets[0].message }}</td>
                    </tr>
                    <tr v-else v-for="asset in filteredAssets" :key="asset.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">
                            <input type="checkbox" :value="asset.id" v-model="selected" @click="toggleSelected"/>   
                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">{{ asset.account_code }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-left">{{ asset.account_title}}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 text-center"><button type="button" @click="initView(asset)" :euser="asset"><ViewIcon /></button></td>
...
export default {
    ...
    methods: {
        ...
        fetchAssets() {
            axios.get(`coa/listTempAsset`)
            .then(response => {
                this.assets = response.data.data;
                console.log(this.assets)
                if (Array.isArray(response.data.list)) {
                this.assets = response.data.list.filter(asset => {
                    return asset.approval_status === 'For Approval - DH'
                });
            }
            })
            .catch(error => {
        ...
        disapproveAcc() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.post(`/coa/disapproveAsset`, { id: this.selected })
                    .then((response) => {
        ...
        forApprovalAcc() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Send approval request for each selected asset
                    axios.post(`coa/AssetforApprovalCa`, { id: this.selected })
                    .then((response) => {
        ...
        toggleSelectAll() {
            ...
            if (this.selectAll) {
                this.selected = this.filteredAssets.map((asset, index) => ({ [`id[${index}]`]: asset.id })); // create array of objects with each object containing the id of the corresponding asset
    ...
    mounted() {
        ...
        this.description = this.euser.currDesc.description;
            ...
            axios.get('coa/listTempAsset').then(response => this.selectedOption = response.data.list[0]['date_effectivity'])
            axios.get('coa/listTempAsset').then(response => this.coa_title = response.data.list[0]['coa_title'])
    ...
    computed: {
        filteredAssets() {
            ...
            const filtered = this.assets.filter((asset) => {
                ...
                ) && asset.approval_status !== 'For Approval - CA';
            });
