                ...
                v-if="this.userDetails === 'City Accountant' || this.userDetails === 'Approver' && this.userPosition === 'Assistant Department Head forFRS'">
                    ...
                    <tr v-else v-for="liability in filteredLiabilities" :key="liability.id">
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 text-center">
                            <input type="checkbox" :value="liability.id" v-model="selected" @click="toggleSelected"/>   
                        </td>
...
export default {
    ...
    methods: {
        ...
        fetchLiabilities() {
            axios.get(`coa/listTempLiabilities`)
            .then(response => {
                ...
                if (Array.isArray(response.data.list)) {
                    this.liabilities = response.data.list.filter(liability => {
                        return liability.approval_status === 'For Approval - CA'
                    });
                }
            })
            .catch(error => {
        ...
        approveAcc() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const selectedLiabilities = this.selected;

                    // Check if any Liabilities is selected
                    if (selectedLiabilities.length === 0) {
                        alert("Please select at least one liability to approve.");
                        return;
                    }

                    // Send approval request for each selected Liabilities
                    axios.post(`coa/liabilityincomeApproveByCa`, { id: this.selected })
                    .then((response) => {
                        console.log(`Liabilities ${response.data.id} has been approved.`);
                        window.location.reload();
                    })
                    .catch((error) => {
                        console.error(`Error approving liability ${error.message}`);
                    });
        ...
        disapproveAcc() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.post(`coa/disapproveLiabilities`, { id: this.selected })
                    .then((response) => {
                        console.log(`Liability ${response.data.id} has been disapproved.`);
                        window.location.reload();
                    })
                    .catch((error) => {
                        console.error(`Error approving liability ${error.message}`);
        ...
        toggleSelectAll() {
            ...
            if (this.selectAll) {
                this.selected = this.filteredLiabilities.map((liability, index) => ({ [`id[${index}]`]: liability.id })); // create array of objects with each object containing the id of the corresponding liability
        ...
        toggleSelected(liability) {
            console.log(liability)
            if (this.isSelected(liability)) {
                const index = this.selected.findIndex((selectedLiability) => {
                    console.log(selectedLiability);
                    return selectedLiability.id === liability.id;
                });
                this.selected.splice(index, 1);
            } else {
                this.selected.push(liability.id);
            }
                this.selectAll = this.selected.length === this.liabilities.length;
        },
    },
    watch: {
        selectAll(newVal) {
            if (newVal) {
            this.selected = this.filteredLiabilities.map(liability => liability.id);
            } else {
            this.selected = [];
            }
        }
    ...
    mounted() {
        axios.get(`coa/listTempLiabilities`)
        .then(response => {
            this.liabilities = response.data.list;
        ...
        this.description = this.euser.currDesc.description;
            ...
            axios.get('coa/listTempLiabilities').then(response => this.selectedOption = response.data.list[0]['date_effectivity'])
            axios.get('coa/listTempLiabilities').then(response => this.coa_title = response.data.list[0]['coa_title'])
    },
    computed: {
        filteredLiabilities() {
            ...
            const filtered = this.liabilities.filter((liability) => {
                ...
                ) && liability.approval_status !== 'For Approval - DH';
            });
