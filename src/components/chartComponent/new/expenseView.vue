...
<script>
import TableView from './table/expenseTable.vue';
import ForApprovalView from './table/expenseTableApproval.vue';
    ...
    methods: {
        ...
        axios.get('coa/showExpenses')
            .then(response => {
                ...
                const existingExpenses = Array.isArray(response.data.data) ? response.data.data : [];
                const existingDateEffectIndexes = existingExpenses.map(expenses => expenses.date_effect_index);
                const fileReader = new FileReader();
                fileReader.onload = () => {
                    const fileData = fileReader.result;
                    const parsedData = Papa.parse(fileData, {header: true}).data;
                    const newDateEffectIndexes = parsedData.map(expenses => expenses.date_effect_index);
                    ...
                    const newexpenses = parsedData.filter(expenses => !existingDateEffectIndexes.includes(expenses.date_effect_index));
                    if (newexpenses.length === 0) {
                        console.log('passed here too')
                        alert('All expenses in the file already exist in the database');
                        return;
                    }
                    ...
                    axios.post('coa/importExpenses', newFormData, {
                        headers: {
            ...
            .catch(error => {
                console.error(error);
                alert('Failed to retrieve existing expenses');
            });
        },
    },
    async created() {
