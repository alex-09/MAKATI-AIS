<script>
import TableView from "./table/assetTable.vue";
import ForApprovalView from "./table/assetTableApproval.vue";
  ...
  methods: {
    ...
    uploadFile() {
      ...
      axios
        .get("coa/showAssets")
        .then((response) => {
          const existingAssets = Array.isArray(response.data.data)
            ? response.data.data
            : [];
          const existingDateEffectIndexes = existingAssets.map(
            (asset) => asset.date_effect_index
          );
          const fileReader = new FileReader();
          fileReader.onload = () => {
            const fileData = fileReader.result;
            const parsedData = Papa.parse(fileData, { header: true }).data;
            const newDateEffectIndexes = parsedData.map(
              (asset) => asset.date_effect_index
            );
            const duplicates = newDateEffectIndexes.filter((index) =>
              existingDateEffectIndexes.includes(index)
            );
            if (duplicates.length > 0) {
              console.log("passed here");
              alert(
                `The following date_effect_indexes already exist in the database: ${duplicates.join(
                  ", "
                )}`
              );
              return;
            }
            const newAssets = parsedData.filter(
              (asset) =>
                !existingDateEffectIndexes.includes(asset.date_effect_index)
            );
            ...
            axios
              .post("coa/importAsset", newFormData, {
                headers: {
...
<template>
      ...
      <div class="flex justify-start gap-8 flex-col text-center mt-8 mx-8">
        <div class="flex flex-col-2 gap-8">
          <div class="flex flex-row gap-4 items-center">
            <span class="text-sm font-semibold"> Chart Title </span>
            <input
              type="text"
              id="coaTitleInput"
              v-model="coaTitle"
          ...
          <div class="flex flex-row gap-4 items-center">
            <span class="text-sm font-semibold"> Date Effectivity </span>
            <input
              type="date"
        ...
        <div class="flex flex-row gap-8 items-center">
          <button
            class="bg-CfirstBtn rounded-md w-[200px] h-42 1536:w-[120px] 1536:h-[42px] flex justify-center items-center text-center uppercase text-white text-sm font-medium hover:bg-[#B0960F]"
            @click="openFileInput"
          >
            File Upload
          </button>
          <div class="flex flex-col gap-4">
            ...
            <span
              ...
              class="text-primary-color font-normal text-sm"
  ...
  <div v-else>
    ...
    <div
      v-if="
        (this.userPosition === 'Division Head' &&
          this.userDivision === 'Financial Reporting Division') ||
        this.userDetails === 'Processor' ||
        this.userDivision === 'Financial Reporting Division'
      "
    >
      <ForApprovalView />
