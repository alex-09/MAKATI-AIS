...
<script>
import TableView from './table/incomeTable.vue';
import ForApprovalView from './table/incomeTableApproval.vue';
    ...
    methods: {
        ...
        axios.get('coa/showIncome')
            .then(response => {
                ...
                const existingIncome = Array.isArray(response.data.data) ? response.data.data : [];
                const existingDateEffectIndexes = existingIncome.map(incomes => incomes.date_effect_index);
                const fileReader = new FileReader();
                fileReader.onload = () => {
                    const fileData = fileReader.result;
                    const parsedData = Papa.parse(fileData, {header: true}).data;
                    const newDateEffectIndexes = parsedData.map(incomes => incomes.date_effect_index);
                    ...
                    const newIncomes = parsedData.filter(incomes => !existingDateEffectIndexes.includes(incomes.date_effect_index));
                    if (newIncomes.length === 0) {
                        console.log('passed here too')
                        alert('All incomes in the file already exist in the database');
                        return;
                    }
                    ...
                    axios.post('coa/importIncome', newFormData, {
                        headers: {
