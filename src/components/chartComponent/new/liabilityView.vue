...
<script>
import TableView from './table/liabilityTable.vue';
import ForApprovalView from './table/liabilityTableApproval.vue';
    ...
    methods: {
        ...
        axios.get('coa/showLiabilities')
            .then(response => {
                ...
                const existingLiabilities = Array.isArray(response.data.data) ? response.data.data : [];
                const existingDateEffectIndexes = existingLiabilities.map(liability => liability.date_effect_index);
                const fileReader = new FileReader();
                fileReader.onload = () => {
                    const fileData = fileReader.result;
                    const parsedData = Papa.parse(fileData, {header: true}).data;
                    const newDateEffectIndexes = parsedData.map(liability => liability.date_effect_index);
                    ...
                    const newLiabilities = parsedData.filter(liability => !existingDateEffectIndexes.includes(liability.date_effect_index));
                    if (newLiabilities.length === 0) {
                        console.log('passed here too')
                        alert('All liabilities in the file already exist in the database');
                        return;
                    }
                    ...
                    axios.post('coa/importLiability', newFormData, {
                        headers: {
            ...
            .catch(error => {
                console.error(error);
                alert('Failed to retrieve existing liabilities');
            });
        },
    },
    async created() {
