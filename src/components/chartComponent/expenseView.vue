<script>
import showExpensesAPI from '../../api_request/chartAccounts/current/expenses'
import PendingView from './pending/pendingExpense.vue';
import PendingApprovalView from "./pending/pendingApprovalExpense.vue";
import TableView from './table/tableExpense.vue';
    ...
    methods: {
        ...
        async createAccount() {
            try {
                ...
                const response = await axios.get("coa/showExpenses");
                const existingAccounts = response.data.list;
                const currentYear = response.data.date;
                const accountCodeExists = existingAccounts.some(account => account.account_code === this.account_code);
                ...
                const createResponse = await axios.post("coa/enrollExpenses", newAccount);
...
<template>
            ...
            <div class="grid grid-cols-7 gap-4 justify-center mb-4">
                <div class="grid grid-cols-1 gap-2">
                    ...
                    <input 
                        ...
                        data-index="0" ref="input-0"
                        @input="focusNextOncePopulated($event, 2); validateNumber($event, 'account_group')"
                        placeholder="02"
                        readonly>
                </div>
                <div class="grid grid-cols-1 gap-2">
                    <span class="text-xs text-center mb-4 font-normal">Major Account Group</span>
                    <input 
                        type="text" 
                        class="h-32 border rounded px-2 pl-3 focus:outline-none focus-visible:ring-white focus:border-input-color border-input-color text-sm text-gray-500"
                        id="major_account_group" 
                        v-model="major_account_group"
                        data-index="1" ref="input-1" 
                        @input="focusNextOncePopulated($event, 2); validateNumber($event, 'major_account_group')" 
                        >
                    <span v-if="major_account_group.length > 2" class="text-xs text-danger-color text-normal">Maximum length is 2 characters.</span>  
                </div>
                <div class="grid grid-cols-1 gap-2">
                    <span class="text-xs text-center mb-4 font-normal">Sub-Major Account Group</span>
                    <input 
                        type="text" 
                        class="h-32 border rounded px-2 pl-3 focus:outline-none focus-visible:ring-white focus:border-input-color border-input-color text-sm text-gray-500"
                        id="sub_major_account_group" 
                        v-model="sub_major_account_group"
                        data-index="2" ref="input-2" 
                        @input="focusNextOncePopulated($event, 2); validateNumber($event, 'sub_major_account_group')" 
                        >
                    <span v-if="sub_major_account_group.length > 2" class="text-xs text-danger-color text-normal">Maximum length is 2 characters.</span>    
                </div>
                <div class="grid grid-cols-1 gap-2">
                    <span class="text-xs text-center mb-4 font-normal">General Ledger Accounts</span>
                    <input 
                        type="text" 
                        class="h-32 border rounded px-2 pl-3 focus:outline-none focus-visible:ring-white focus:border-input-color border-input-color text-sm text-gray-500"
                        id="general_ledger_accounts" 
                        v-model="general_ledger_accounts"
                        data-index="3" ref="input-3" 
                        @input="focusNextOncePopulated($event, 3); validateNumber($event, 'general_ledger_accounts')" 
                        >
                    <span v-if="general_ledger_accounts.length > 3" class="text-xs text-danger-color text-normal">Maximum length is 3 characters.</span>
                </div>
                <div class="grid grid-cols-1 gap-2">
                    <span class="text-xs text-center mb-4 font-normal">Sub-Ledger Accounts</span>
                    <input 
                        type="text" 
                        class="h-32 border rounded px-2 pl-3 focus:outline-none focus-visible:ring-white focus:border-input-color border-input-color text-sm text-gray-500"
                        id="sub_ledger_accounts" 
                        v-model="sub_ledger_accounts"
                        data-index="4" ref="input-4" 
                        @input="focusNextOncePopulated($event, 3); validateNumber($event, 'sub_ledger_accounts')"
                        >
                    <span v-if="sub_ledger_accounts.length > 3" class="text-xs text-danger-color text-normal">Maximum length is 3 characters.</span>
                </div>
                <div class="grid grid-cols-1 gap-2">
                    <span class="text-xs text-center mb-4 font-normal">Sub-Sub-Ledger Accounts</span>
                    <input 
                        type="text" 
                        class="h-32 border rounded px-2 pl-3 focus:outline-none focus-visible:ring-white focus:border-input-color border-input-color text-sm text-gray-500"
                        id="sub_sub_ledger_accounts" 
                        v-model="sub_sub_ledger_accounts"
                        data-index="5" ref="input-5" 
                        @input="focusNextOncePopulated($event, 3); validateNumber($event, 'sub_sub_ledger_accounts')" 
                        >
                    <span v-if="sub_sub_ledger_accounts.length > 3" class="text-xs text-danger-color text-normal">Maximum length is 3 characters.</span>
                </div>
                <div class="grid grid-cols-1 justify-items-center items-center">
                    <button @click="createAccount" class="w-100 h-[30px] bg-dashboardBtn9 uppercase rounded text-base font-semibold ">Add</button>
                </div>
            </div>
            <div class="grid grid-cols-7 gap-4 justify-center mt-4">
                <div class="col-span-4">
                    <div class="grid grid-cols-1">
                        <span class="text-xs text-center mb-4 font-normal">Account Title</span>
                        <input 
                            type="text" 
                            class="h-32 border rounded px-2 pl-3 focus:outline-none focus-visible:ring-white focus:border-input-color border-input-color text-sm text-gray-500"
                            id="account_title" 
    ...
    <div v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division' ">
        <PendingApprovalView />
    </div>
</template>