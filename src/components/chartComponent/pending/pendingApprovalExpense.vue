        ...
        <div class="overflow-y-auto overscroll-y-auto overflow-x-hidden scrollbar w-1850 1536:w-1458 xxxxl:w-1850">
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8 border-2 border-dashboardBtn9">
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="expense in expenses" :key="expense.id" if="expense.status === 'For Approval - DH'">
...
export default {
    ...
    mounted() {
        ...
        .then(response => {
            this.expenses = response.data.list.filter(expense => expense.status === 'For Approval - DH');
    ...
    computed: {
        pendingExpenses() {
            return this.expenses.filter(expense => expense.status === 'For Approval - DH');
        },
    },
    methods: {
        closeModal() {
        ...
        approveAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/approveExpensesAccountDh/${id}`)
                    .then(response => {
