        ...
        <div class="overflow-y-auto overscroll-y-auto overflow-x-hidden scrollbar w-1850 1536:w-1458 xxxxl:w-1850">
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8 border-2 border-dashboardBtn9">
                ...
                <tbody class="divide-y divide-gray-200" v-if="this.userDetails === 'City Accountant' || this.userPosition === 'Assistant Department Head forFRS'">
                    <tr v-for="liabilty in liabilities" :key="liabilty.id" if="liabilty.status == 'For Approval - CA'">
...
export default {
    ...
    mounted() {
        ...
        .then(response => {
            this.liabilities = response.data.list.filter(liabilty => liabilty.status === 'For Approval - CA');
    ...
    computed: {
        pendingLiabilities() {
            return this.liabilities.filter(liabilty => liabilty.status === 'For Approval - CA');
        },
    },
    methods: {
        closeModal() {
        ...
        approveAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/approveLiabilitiesAccountCa/${id}`)
                    .then(response => {
