        ...
        <div class="overflow-y-auto overscroll-y-auto overflow-x-hidden scrollbar w-1850 1536:w-1458 xxxxl:w-1850">
            <table class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8 border-2 border-dashboardBtn9">
                ...
                <tbody class="divide-y divide-gray-200" v-if="this.userDetails === 'City Accountant' || this.userPosition === 'Assistant Department Head forFRS'">
                    <tr v-for="asset in assets" :key="asset.id" if="asset.status ==='For Approval - CA'">
                        ...
                        <div>
                            <td class="px-6 py-4 text-sm text-gray-800 text-center uppercase flex gap-2 justify-center">       
                                <button @click="approveAcc(asset.id)" class="w-[127px] h-30 rounded uppercase text-base font-medium text-white bg-save-color">approve</button>
...
export default {
    ...
    mounted() {
        ...
        .then(response => {
            this.assets = response.data.list.filter(asset => asset.status === 'For Approval - CA');
    ...
    computed: {
        pendingAssets() {
            return this.assets.filter(asset => asset.status === 'For Approval - CA');
        },
    },
    methods: {
        closeModal() {
        ...
        approveAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/approveAssetAccountCa/${id}`)
                    .then(response => {
