...
export default {
    ...
    mounted() {
        ...
        .then(response => {
            this.incomes = response.data.list.filter(income => income.status === 'For Approval - DH');
        })
        .catch(error => {
            console.log(error);
        });
    ...
    computed: {
        pendingIncomes(){
            return this.incomes.filter(income => income.status === 'For Approval - DH');
        },
    },
    methods: {
        closeModal() {
        ...
        approvalAcc(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`coa/approveIncomeAccountDh/${id}`)
                    .then(response => {
    ...
    async created() {
        ...
        console.log('Pending Component Here')
    }
}
</script>
...
<template>
    <div 
        class="container grid justify-items-center p-0 my-8"
        v-if="pendingIncomes.length > 0">
        <div 
            class="overflow-y-auto overscroll-y-auto overflow-x-hidden scrollbar w-1850 1536:w-1458 xxxxl:w-1850">
            <table 
                class="divide-y w-[1440px] xxxxl:w-1850 divide-gray-200 mr-8 border-2 border-dashboardBtn9">
                <thead>
                    <tr>
                        <th
                            colspan="4"
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="income in incomes" :key="income.id" if="income.status === 'For Approval - DH'">
                        ...
                        <div v-if="this.userPosition === 'Division Head' && this.userDivision === 'Financial Reporting Division'">
                            <td class="px-6 py-4 text-sm text-gray-800 text-center uppercase flex gap-2 justify-center">       
                                <button @click="approvalAcc(income.id)" class="w-[127px] h-30 rounded uppercase text-base font-medium text-white bg-save-color">approve</button>
                                <button @click="disapproveAcc(income.id)" class="w-[127px] h-30 rounded uppercase text-base font-medium text-white bg-danger-color">disapprove</button>
                            </td>
                        </div>
                    </tr>
                </tbody>
