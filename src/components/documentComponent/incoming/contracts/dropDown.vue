...
<template>
    <Listbox >
        ...
        >{{ selectedData }}
        <span v-if="noDataSelected" class="text-text-gray font-normal"> {{ placeholder }}</span>
        ...
        <transition
            ...
            >   
                ...
                @click="selectedData = option.label, noDataSelected = false"
                >
                <li
                    :class="[
                    
                    selected ? 'bg-dashboardBtn3 z-20 absolute '  : ' hover:text-white hover:bg-gray-400',
                        'relative  px-[22px] py-1 select-none' , ]" >
                    <span class=" font-normal text-sm">{{ option.label }}</span>
                </li>
                </ListboxOption>
            </ListboxOptions>
        </transition>
