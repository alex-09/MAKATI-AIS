...
export default {
    ...
    methods: {
        fetchLists() {
            axios.get("appropriation/listApproForReview")
                .then((response) => {
                    this.lists = response.data.list;
                    console.log(this.lists);
                    if (Array.isArray(response.data.list)) {
                        this.lists = response.data.list.filter((list) => {
                            return list.status === 'For Review' && list.type === 'New'
                        });
                    }
                })
                .catch((error) => {
        ...
        filteredLists() {
            ...
            const filtered = this.lists.filter((list) => {
                return (
                    ...
                    list.status === 'For Review' && list.type === 'New'
                );
            });
...
<template>
    <div class="flex justify-between items-center m-8">
        <span class="text-[22px] font-bold text-[#0E4572]">Enroll/New Appropriation - For Review</span>
        <div class="mx-3 my-3">
