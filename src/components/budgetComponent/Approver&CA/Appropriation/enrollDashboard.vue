...
export default {
  ...
  methods: {
    fetchLists() {
      axios
        .get("appropriation/listApproDivisionHead")
        .then((response) => {
          this.lists = response.data.list;
          console.log(this.lists);
          if (Array.isArray(response.data.list)) {
            this.lists = response.data.list.filter((list) => {
              return list.status === "FOR APPROVAL - DH" && list.type === "New";
            });
          }
        })
        .catch((error) => {
...
<template>
  <div class="flex justify-center items-center m-8 relative">
    <span class="absolute left-0 text-[22px] font-bold text-[#0E4572] uppercase"
      >approval of appropriation</span
    >
    <div class="flex gap-[80px]">
      <RouterLink
        ...
        :class="{
          'border-b-4 border-primary-color': [
            '/Budget-Management/Executive-Funds/Approver&CA-Enroll-Dashboard',
          ].includes($route.path),
        }"
      >
        new
      ...
      <RouterLink
        ...
        :class="{
          'border-b-4 border-primary-color': [
            '/Budget-Management/Executive-Funds/Approver&CA-Update-Dashboard',
          ].includes($route.path),
        }"
      >
        update
