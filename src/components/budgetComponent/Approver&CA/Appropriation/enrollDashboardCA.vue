...
export default {
    ...
    methods: {
        fetchLists() {
            axios.get("appropriation/listApproCA")
                .then((response) => {
                    this.lists = response.data.list;
                    console.log(this.lists);
                    if (Array.isArray(response.data.list)) {
                        this.lists = response.data.list.filter((list) => {
                            return list.status === 'FOR APPROVAL - CA' && list.type === 'New';
                        });
                    }
                })
                .catch((error) => {
        ...
        pushToListView(listId, AIPcode) {
            ...
            router.push({
                name: 'Details for Approval Appropriation - Reviewer', // Replace with the actual name of the target route
                query: {
                id: listId,
                aipcode: AIPcode,
                }
...
<template>
    <div class="flex justify-center items-center m-8 relative">
        <span class="absolute left-0 text-[22px] font-bold text-[#0E4572] uppercase">approval of appropriation</span>
        <div class="flex gap-[80px]">
            ...
            :class="{'border-b-4 border-primary-color': ['/Budget-Management/Executive-Funds/Approver&CA-Enroll-Dashboard'].includes($route.path) }">
                new
            </RouterLink>
            ...
            class="text-[14px] font-medium text-primary-color uppercase w-[100px] h-[24px] hover:border-b-4 hover:border-primary-color cursor-pointer flex justify-center"
            :class="{'border-b-4 border-primary-color': ['/Budget-Management/Executive-Funds/Approver&CA-Update-Dashboard'].includes($route.path) }">
                update
            </RouterLink>
        </div>
    </div>
